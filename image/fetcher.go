...
// TODO : writer should be passed in as a colorized writer
func NewFetcher(w io.Writer, docker *client.Client) (*Fetcher, error) {
...
func (f *Fetcher) pullImage(ctx context.Context, imageID string) error {
	...
	termFd, isTerm := term.GetFdInfo(f.writer)
	err = jsonmessage.DisplayJSONMessagesStream(rc, f.writer, termFd, isTerm, nil)
	if err != nil {
		return err
	}
